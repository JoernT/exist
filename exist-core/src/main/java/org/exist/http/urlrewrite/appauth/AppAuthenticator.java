    ...
    */
    public Subject authenticate(HttpServletRequest request,
        ...
        try (final DBBroker broker = pool.get(Optional.ofNullable(user))) {
            ...
            // get token from cookie if any
            String token = getCookieValue(request);

            // check for a logout as first thing as it should happen even if users have deleted cookies.
            ...
            if (token != null) {
                subject = isTokenValid(token, broker, appName);
                if (subject != null) {
    ...
    private String getUserNameFromToken(String token) {
        String[] al = AuthTokenFactory.getInstance().getFields(token);
        if (al.length != 0) {
            return al[0];
        }
        return null;
    ...
    private Subject isTokenValid(String token, DBBroker broker, String appName) throws EXistException, AuthenticationException {

        // username=null if token cannot be validated
        String username = AuthTokenFactory.getInstance().validateToken(token, appName);
        if (username != null) {
            Logger.log("debug", "token is valid");
            UserAuth userAuth = UserAuth.getInstance();
            LoginDetails details = userAuth.getUserDetails(username);
            // do login - when details == null we came from a logout redirect and user data are already gone
            if (details != null) {
                return existdbLogin(username, details.getPass(), broker);
            }

        } else {
            Logger.log("notice", "token is INVALID or non-existent");
